...
const globalForReporter = globalThis as unknown as { __pipulseReporter?: ErrorReporter };

/** Attach an external error reporter (see the Sentry recipe above). */
export function setErrorReporter(reporter: ErrorReporter): void {
  globalForReporter.__pipulseReporter = reporter;
}
...
export function reportError(msg: string, err: unknown, extra: Record<string, unknown> = {}): void {
  ...
  try {
    globalForReporter.__pipulseReporter?.(err, { msg, ...extra });
  } catch {
