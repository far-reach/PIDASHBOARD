...
import { resolveAgainstPrice } from "@/lib/signals/outcome";
...
describe("resolveOpenSignals against the real schema", () => {
  ...
  it("no event while price is inside the range", async () => {
  ...
  it("writes hit_tp with the triggering price + source, exactly once", async () => {
    const s = await newSignal();
  ...
  it("writes hit_sl on stop breach for shorts", async () => {
  ...
  it("expires a signal past expires_at at market price", async () => {
    const s = await newSignal({
      issuedAt: new Date(Date.now() - 2 * 3600_000).toISOString(),
      expiresAt: new Date(Date.now() - 3600_000).toISOString(),
    });
    await resolveOpenSignals(tick(0.52), db);
    const stored = await getSignalWithOutcome(s.id, db);
    expect(stored?.status).toBe("expired");
    expect(stored?.exitPrice).toBe(0.52);
  });
